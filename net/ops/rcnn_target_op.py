...
def draw_rcnn_labels(image, rois,  labels):
    is_print=0
    ...
    if 1:
        for i in bg_label_inds:
            a = boxes[i]
            cv2.rectangle(image,(a[0], a[1]), (a[2], a[3]), (32,32,32), 1)
            cv2.circle(image,(a[0], a[1]),2, (32,32,32), -1)

    for i in fg_label_inds:
        a = boxes[i]
        cv2.rectangle(image,(a[0], a[1]), (a[2], a[3]), (255,0,255), 1)
        cv2.circle(image,(a[0], a[1]),2, (255,0,255), -1)



def draw_rcnn_targets(image, rois, labels,  targets):
    is_print=0
